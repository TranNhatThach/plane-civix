...
export default function ChangelogPage() {
  ...
  const currentRelease = CIVIX_CHANGELOG_RELEASES.find((r) => r.id === selectedReleaseId);

  return (
    <div className="bg-custom-background-100 text-custom-text-100 vertical-scrollbar relative flex min-h-screen w-full flex-col overflow-y-auto">
      {/* Official Plane Top Navigation Bar */}
      ...
      <div className="flex-1">
        {selectedReleaseId && currentRelease ? (
          ...
          <PlaneChangelogTimeline releases={CIVIX_CHANGELOG_RELEASES} onSelectRelease={handleSelectRelease} />
        )}
      </div>

      {/* Minimal Clean Footer */}
      <footer className="border-custom-border-200/60 bg-custom-background-90/50 text-xs text-custom-text-300 border-t py-10 text-center">
        <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-4 px-4 sm:flex-row">
          <div className="text-custom-text-200 flex items-center gap-2 font-semibold">
            <svg
              className="text-custom-text-100 h-4 w-4 fill-current"
              viewBox="0 0 24 24"
            ...
            <span>Plane / Civix Edition</span>
          </div>
          <p>© 2026 Plane Software, Inc. & Civix. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
